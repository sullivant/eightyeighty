use crate::{
    bus::Bus, constants::{FLAG_CARRY, FLAG_PARITY, FLAG_SIGN, FLAG_ZERO}, cpu::{CPU, make_pointer}
};
...
impl CPU {
    ...
    /// return is performed
    pub fn rpo(&mut self, bus: &Bus) -> Result<u8, String> {
    ...
    /// return is performed
    pub fn rpe(&mut self, bus: &Bus) -> Result<u8, String> {
        if self.test_flag(FLAG_PARITY) {
            self.ret(bus)?;
            Ok(11)
        } else {
    ...
    /// return is performed
    pub fn rm(&mut self, bus: &Bus) -> Result<u8, String> {
    ...
    pub fn rp(&mut self, bus: &Bus) -> Result<u8, String> {
        if self.test_flag(FLAG_SIGN) {
            Ok(5)
        } else {
            self.ret(bus)?;
    ...
    pub fn rc(&mut self, bus: &Bus) -> Result<u8, String> {
    ...
    pub fn rnc(&mut self, bus: &Bus) -> Result<u8, String> {
        if self.test_flag(FLAG_CARRY) {
            Ok(5)
        } else {
            self.ret(bus)?;
    ...
    pub fn rz(&mut self, bus: &Bus) -> Result<u8, String> {
    ...
    pub fn rnz(&mut self, bus: &Bus) -> Result<u8, String> {
        if self.test_flag(FLAG_ZERO) {
            Ok(5)
        } else {
            self.ret(bus)?;
    ...
    pub fn cc(&mut self, dl: u8, dh: u8, bus: &mut Bus) -> Result<u8, String> {
        if self.test_flag(FLAG_CARRY) {
            self.call(dl, dh, bus)?;
            Ok(17)
        } else {
    ...
    pub fn cnc(&mut self, dl: u8, dh: u8, bus: &mut Bus) -> Result<u8, String> {
        if self.test_flag(FLAG_CARRY) { 
            Ok(11)
        } else {
            self.call(dl, dh, bus)?;
    ...
    pub fn cnz(&mut self, dl: u8, dh: u8, bus: &mut Bus) -> Result<u8, String> {
        if self.test_flag(FLAG_ZERO) {
            self.call(dl, dh, bus)?;
            Ok(17)
        } else {
    ...
    pub fn cz(&mut self, dl: u8, dh: u8, bus: &mut Bus) -> Result<u8, String> {
        if self.test_flag(FLAG_ZERO) {
            Ok(11)
        } else {
            self.call(dl, dh, bus)?;
    ...
    pub fn cm(&mut self, dl: u8, dh: u8, bus: &mut Bus) -> Result<u8, String> {
        if self.test_flag(FLAG_SIGN) {
            self.call(dl, dh, bus)?;
            Ok(17)
        } else {
    ...
    pub fn cp(&mut self, dl: u8, dh: u8, bus: &mut Bus) -> Result<u8, String> {
        if self.test_flag(FLAG_SIGN) {
            Ok(11)
        } else {
            self.call(dl, dh, bus)?;
    ...
    pub fn cpe(&mut self, dl: u8, dh: u8, bus: &mut Bus) -> Result<u8, String> {
        if self.test_flag(FLAG_PARITY) {
            self.call(dl, dh, bus)?;
            Ok(17)
        } else {
    ...
    pub fn cpo(&mut self, dl: u8, dh: u8, bus: &mut Bus) -> Result<u8, String> {
        if self.test_flag(FLAG_PARITY) {
            Ok(11)
        } else {
            self.call(dl, dh, bus)?;
    ...
    pub fn call(&mut self, dl: u8, dh: u8, bus: &mut Bus) -> Result<u8, String> {
...
mod tests {
    ...
    use crate::{
        bus::Bus, constants::{FLAG_CARRY, OPCODE_SIZE}, cpu::CPU, memory::Memory
    };
