...
impl CPU {
    ...
    pub fn lhld(&mut self, dl: u8, dh: u8) -> Result<(), String> {
        ...
        self.l = match self.memory.get(addr as usize) {
            Some(&v) => v,
            None => return Err(format!("LHLD: Unable to read for L in memory at {:#04X}",addr)),
        };
        addr = addr.overflowing_add(0x01).0;
        self.h = match self.memory.get(addr as usize) {
            Some(&v) => v,
            None => return Err(format!("LHLD: Unable to read for H in memory at {:#04X}",addr)),
        };
